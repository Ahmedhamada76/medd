...
function initHeroMedia() {
    ...
    const heroVideoContainer = heroVideo.closest('.hero-video-container');
    ...
    const revealHeroVideo = () => {
        ...
        heroVideoContainer?.style.removeProperty('background-image');
    };
    ...
    const heroObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting && heroVideo.dataset.videoLoaded === 'true') {
                heroVideo.play().catch(() => {});
            } else {
                heroVideo.pause();
            }
    ...
    requestAnimationFrame(() => {
        window.setTimeout(tryStartHeroVideo, 80);
    });

    window.addEventListener('focus', tryStartHeroVideo, { passive: true });
...
function initVideoLazyLoading() {
    ...
    document.querySelectorAll('[data-lazy-video]').forEach(video => observer.observe(video));
}
